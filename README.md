...
One font pixel is 100 units of a 1200 unit em, so the em is 12 font pixels: 10 above the
...
reach one font pixel below the em and accents on capitals one above it; a container with
...
The letters A to Z and a to z are Mia's original drawings in `glyphs/sheet.png`. Everything else lives as text bitmaps in
...
`build:font` traces the sheet into `dist/FalseType.otf` and `.woff2`. To change one of the
letters, edit `glyphs/sheet.png` directly at 1:1 and run `build:font` only.
